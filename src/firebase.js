...
async function getFieldsOfUser(user_id, email) {
    const _query1 = await admin.firestore().collection('fields').where(
        'user_id', '==', user_id
    ).get();
    const _query2 = await admin.firestore().collection('fields').where(
        'associates', 'array-contains', email
    ).get();
...
async function updateField(field_id, updates, user_id){
    if ('device_id' in updates){
        await admin.firestore().collection('devices').doc(updates.device_id_last).update({
            field_id: admin.firestore.FieldValue.delete(),
            user_id: admin.firestore.FieldValue.delete()
        });


        await admin.firestore().collection('devices').doc(updates.device_id).update({
            field_id: field_id,
            user_id: user_id
        });
