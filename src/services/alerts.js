...
const { culturasParams } = require("../data/fieldsParams");
const { verifyField, getUserData } = require("../firebase");
const { generateAlertEmail } = require("../templates/emails");
const { inInterval } = require("../utils");
        ...
        if (currentSensParams) {
            ...
            if (user){
                var alertas = [
                    ...
                    !inInterval(data.umidade_ambiental, currentSensParams.umidade_ambiental) ? {
                ...
                if (alertas.length > 0){
                   const emailContent= generateAlertEmail(user.nome, data.moment.toDate(), alertas);
